...
impl ProducerComponent {
    ...
    fn create_instance(&self, conn_id: u128, any_sender: AnySender) {
        if let Ok(session_sender) = Arc::clone(&any_sender).downcast::<Sender<EchoCmd>>() {
            ...
            send_cmd(&session_sender, EchoCmd::Instance(conn_id, sender, settings::Component::EchoProducer));
        } else {
...
impl Machine<ComponentCmd> for ProducerComponent {
    ...
    fn receive(&self, cmd: ComponentCmd) {
        match cmd {
            ComponentCmd::NewSession(conn_id, service, any_sender) if service == Service::EchoServer => self.create_instance(conn_id, any_sender),
            ComponentCmd::Start => (),
            ComponentCmd::Stop => (),
            _=> (),
        }
...
impl EchoInstance {
    ...
    fn new_data(&self, conn_id: u128, bytes: &EchoData) {
        self.mutable.lock().unwrap().senders.iter().for_each(|s| send_cmd(&s, EchoCmd::NewData(conn_id, Arc::clone(bytes))));
    }
}
