...
pub fn configure(settings: &Settings, components: &[ComponentInfo]) -> Result<Option<ComponentSender> ,Box<dyn Error>> {
    ...
    for c in &settings.coordinator {
        if let Some(v) = c.get(&Coordinator::EchoCoordinator) {
            ...
            if coordinator.is_none() { continue }
            ...
            log::debug!("echo server selected configuration: {:#?}", v);
    ...
    log::warn!("echo server coordinator didn't find its configuration.");
    Ok(None)
}
...
impl EchoCoordinator {
    fn create_instance(&self, conn_id: u128, buf_size: usize) {
        ...
        let (instance, sender) = executor::connect::<_,EchoCmd>(EchoInstance::new(conn_id, buf_size, self.net_sender.clone()));
        ...
        self.components.iter().for_each(|c| send_cmd( &c.sender, ComponentCmd::NewSession(conn_id, Service::EchoServer, Arc::new(sender.clone()))));
...
impl EchoInstance {
    fn new(conn_id: u128, buf_size: usize, net_sender: NetSender) -> Self {
        Self { conn_id, buf_size, net_sender, my_sender: Mutex::default(), consumer: Mutex::default(), producer: Mutex::default() }
    }

    fn close(&self, _conn_id: u128) {
    ...
    fn received_bytes(&self, conn_id: u128, bytes: Vec<u8>) {
        ...
        send_cmd(self.consumer.lock().unwrap().as_ref().unwrap(), EchoCmd::NewData(conn_id, bytes));
    }
}
