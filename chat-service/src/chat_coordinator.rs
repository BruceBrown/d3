...
//! response, it tells the network where to send control and data, via `NetCmd::BindConn`. For the chat server,
...
pub fn configure(settings: &Settings, components: &[ComponentInfo]) -> Result<Option<ComponentSender>, ComponentError> {
    ...
    log::warn!("The configuration for the chat server coordinator was not found.");
    Err(ComponentError::BadConfig(
        "The configuration for the chat server coordinator was not found.".to_string(),
    ))
}
