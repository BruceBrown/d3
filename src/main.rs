...
fn main() {
    ...
    if settings.dev_mode { log::error!("Running with dev_mode enabled") }

    // apply any tuning configuration
    if let Some(e) = settings.executor.as_ref() {
        if let Some(c) = e.queue_size.as_ref() { executor::set_default_channel_capacity(*c) }
        if let Some(c) = e.executors.as_ref() { executor::set_executor_count(*c) }
        if let Some(c) = e.time_slice.as_ref() { executor::set_time_slice(std::time::Duration::from_millis(*c as u64)) }
    };
...
fn run_server(settings: &settings::Settings) {
    ...
    components.iter().for_each(|c| if c.sender.send(ComponentCmd::Start).is_err() {
        log::warn!("unable to start component {:#?}", c.component)
    });

    coordinators.iter().for_each(|c| if c.sender.send(ComponentCmd::Start).is_err() {
